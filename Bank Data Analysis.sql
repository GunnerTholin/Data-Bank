...
--8. Comparing the closing balance of a customer’s first month and the closing balance from their second nth, what percentage of customers:
...
--9.Have a negative first month balance?
SELECT *
FROM q5_sequence
WHERE sequence = 1 
...
--10.Have a positive first month balance?
SELECT 
  ROUND(100 * COUNT(*)::NUMERIC / 
    (SELECT COUNT(DISTINCT customer_id)
...
--11.Increase their opening month’s positive closing balance by more than 5% in the following month?
...
--12.Reduce their opening month’s positive closing balance by more than 5% in the following month?
WITH next_balance_cte AS (
  SELECT 
    customer_id, ending_month, closing_balance, 
...
--13.Move from a positive balance in the first month to a negative balance in the second month?
WITH next_balance_cte AS (
  SELECT 
    customer_id, ending_month, closing_balance, 
    LEAD(closing_balance) OVER (PARTITION BY customer_id ORDER BY ending_month) AS next_balance
